<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<header>
    <div class="top_header">
        <div class="top_header1"></div>
        <div class="top_header2">
            <div class="logo"><a href="#"><img src="/resources/img/Santa_Logo.png" alt=""></a></div>
        </div>
        <div class="top_header3">
            <div class="login_wrapper1"></div>
            <div class="login_wrapper2">
                <div class="login"><a href="#">LOGIN</a></div>
                <div class="join"><a href="#">JOIN</a></div>
            </div>
        </div>
    </div>
    <div class="bottom_header">
        <div class="bottom_header1"></div>
        <div class="bottom_header2">
            <div class="nav1"><a href="#">등산정보</a></div>
            <div class="nav2"><a href="#">등산매칭</a></div>
            <div class="nav3"><a href="#">커뮤니티</a></div>
        </div>
        <div class="bottom_header3"></div>
    </div>
</header>